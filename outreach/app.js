(() => {
  const API_BASE =
    'https://msratyvmnuvozuthgkmi.supabase.co/functions/v1/outreach-api';
  ...
  function openAddIndustryModal() {
    ...
    submitBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        closeModal();
        if (slug) {
          location.href = '/outreach/industry/' + encodeURIComponent(slug) + '?tab=templates';
        } else {
          location.reload();
        }
      } catch (err) {
  ...
  async function renderIndustrySidebar(active, email) {
    ...
    nav.innerHTML = `
      ...
      <a data-nav="dashboard" href="/outreach/">Dashboard</a>
      ...
      <a data-nav="controls" href="/outreach/controls">Controls</a>
      <a data-nav="stats" href="/outreach/stats">Stats</a>
    ...
    industriesEl.innerHTML = list
      .map((ind) => {
        ...
        return `<a class="nav-industry-link${isActive ? ' active' : ''}${archived ? ' archived' : ''}"
            href="/outreach/industry/${esc(ind.slug)}">

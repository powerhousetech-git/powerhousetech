(function () {
  ...
  async function loadSheetLeads(force) {
    ...
    if (!res.ok) {
      toast((res.data && (res.data.error || res.data.message)) || 'Could not load master sheet', true);
      return state.leads || [];
    }
  ...
  function renderView(v) {
    ...
    else if (v === 'lead-tracker') renderLeadTracker();
  ...
  async function renderCapture() {
    var main = $('main-content');
    var tab = state.captureTab || 'excel';
  ...
  function mapOcrContact(c) {
    c = c || {};
    return {
      name: c.name || c.Name || c.full_name || '',
      full_name: c.name || c.Name || c.full_name || '',
      email: String(c.email || c.Email || '').toLowerCase().trim(),
      phone: c.phone || c.Phone || '',
      company: c.company || c.Company || '',
      designation: c.designation || c.Designation || c.title || '',
  ...
  async function autoImportOcrLeads(extractedLeads) {
  ...
  function renderOcrSummary(result, extractedLeads) {
    ...
    if (!resultsEl) return;
    var lines = (extractedLeads || []).map(function(l) {
      return '<li style="margin:0 0 6px">' +
        '<strong>' + esc(l.full_name || l.name || '—') + '</strong>' +
        (l.email ? ' · ' + esc(l.email) : '') +
    ...
    resultsEl.innerHTML =
      '<div class="panel" style="padding:14px">' +
        ...
        '<p style="margin:10px 0 0;font-size:12px;color:var(--muted)">' +
          ...
          (result.skipped ? ' · ' + result.skipped + ' incomplete skipped' : '') +
  ...
  async function handlePdfFiles(fileList) {
    ...
    if (status) status.textContent = 'Saving ' + extractedLeads.length + ' contact(s) to master sheet…';
    var result = await autoImportOcrLeads(extractedLeads);
    state.sheetFetchedAt = null;

    if (result.ok) {
      toast('Added ' + result.ok + ' lead(s) from card scan' +
        (result.dup ? ' · ' + result.dup + ' duplicate(s) skipped' : '') +
        (result.fail ? ' · ' + result.fail + ' failed' : ''));
      ...
      if (status) status.textContent = 'No new leads saved.';
      renderOcrSummary(result, extractedLeads);
    }
  }
  ...
  async function renderLeads() {
    ...
    main.innerHTML =
      ...
      '<div class="filter-bar">' +
        ...
        '<span class="filter-note" style="font-size:13px;color:var(--muted)">All statuses shown</span>' +
  ...
  async function renderReviewDrafts() {
    ...
    var allEmails = Array.isArray(res.data) ? res.data
      : (res.data && Array.isArray(res.data.data) ? res.data.data : []);
    var drafts = allEmails.filter(function(e) {
      return e.status === 'pending_review' || e.is_ai_draft === true || e.is_ai_draft === 'TRUE';
    });
  ...
  async function renderSettings() {
    ...
    var rawSettings = Array.isArray(res.data) ? res.data
      : (res.data && Array.isArray(res.data.data) ? res.data.data : []);
    var s = {};
    ...
    main.innerHTML =
      ...
      '<div class="settings-card">' +
        '<h3>Booking link</h3>' +
        '<label class="field-label">URL<input id="booking-link" value="' + esc(s.booking_link || s.booking_url || '') + '" placeholder="https://calendly.com/…" /></label>' +
        '<button class="btn btn-primary btn-sm" onclick="window.PS2App.saveBookingLink()">Save booking link</button>' +
      '</div>' +
      '<div class="settings-card">' +
        '<h3>AI prompts (optional)</h3>' +
  ...
  async function submitAddLead() {
    ...
    if (body.email) {
      var existingLeads = await loadSheetLeads(false);
      if (PS2Sheet.findLeadByEmail(existingLeads, body.email)) {
  ...
  async function submitAddProject() {
    toast('Projects tracker retired', true); return;
  ...
  async function submitAddUser() {
    toast('User management retired', true); return;
  ...
  function openAddSheet() {
    openModal('<div class="modal-card"><h2>Connect Google Sheet</h2><div class="form-panel">' +
      ...
      '<p style="font-size:12px;color:var(--muted);margin:-4px 0 10px">Try a sample: ' +
        '<button type="button" class="btn btn-sm btn-ghost" onclick="document.getElementById(\'ns-url\').value=\'https://docs.google.com/spreadsheets/d/1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms/edit\'">Google sample sheet</button> ' +
        '<button type="button" class="btn btn-sm btn-ghost" onclick="document.getElementById(\'ns-url\').value=\'https://docs.google.com/spreadsheets/d/1UxKqqC5unE3CwTMqgpB3SMARfxIIw2sVSZQUuz3SclU/edit\'">Field-team demo link</button>' +
      '</p>' +
  ...
  async function submitAddSheet() {
    ...
    if (!res.ok) {
      ...
      if (res.status === 409 || /duplicate/i.test(err)) toast('Duplicate sheet — this Google Sheet + tab is already connected', true);
      else toast(err, true);
      return;
    }
  ...
  async function submitLeadUpload() {
    toast('Lead attachments via Supabase are retired. Add notes on the lead or use Excel import.', true);
  }
  ...
  async function deleteSheet(id) {
    ...
    if (!res.ok) { toast((res.data && res.data.error) || 'Could not delete sheet', true); return; }
    toast('Sheet connection removed');
    refreshSheetLists();
  }

  function refreshSheetLists() {
    if (state.view === 'sheets') renderSheets();
    else if (state.view === 'capture') { state.captureTab = 'sheets'; renderCapture(); }
    else if (state.captureTab === 'sheets') renderCapture();
  }
  ...
  window.PS2App = {
    ...
    deactivateUser: deactivateUser, toggleSheet: toggleSheet, deleteSheet: deleteSheet,
